...
export default async function handler(req, res) {
  ...
  if (isPro) {
    ...
    if (scriptsGenerated >= 3) {
      return res.status(403).json({ error: 'limit_reached', message: "You've used your 3 free scripts. Upgrade to Pro to keep generating." });
    }
  }
  ...
  const animStyleDesc = {
    ...
    '3d-realistic': 'Hyper-realistic 3D animation — characters look like real humans filmed on a real camera but exist in a 3D-rendered world. Natural skin with pores, subsurface scattering, realistic hair strands, real fabric wrinkles, natural body proportions. Think Unreal Engine 5 MetaHuman or a high-end Apple commercial. NOT video-game CGI — this should be indistinguishable from live action at first glance.',
...
${animStyle === 'stick-figure' ? '- A stick figure character with specific distinguishing features (hat, glasses, hair shape, clothing outline). Keep it simple — stick figures should LOOK like stick figures.' : animStyle === 'motion-graphics' ? '- A simplified iconic character or avatar that fits motion graphics style. Not detailed — clean, geometric, professional.' : `- Describe the character's design in ${animStyle === '3d-pixar' ? 'Pixar style (rounded features, oversized expressive eyes, stylized proportions)' : animStyle === '2d-flat' ? '2D flat style (clean shapes, bold outlines, flat colors, simple features)' : 'hyper-realistic 3D style (looks like a real person — natural skin, real hair, real fabric, natural proportions, NOT video-game CGI)'}.`}
...
${animStyle === 'stick-figure' ? '- End every prompt with: stick figure animation, white background, clean black lines, hand-drawn style, smooth animation, no watermark, no text.' : animStyle === 'motion-graphics' ? '- End every prompt with: motion graphics, clean vector design, smooth transitions, professional animation, no watermark, no text.' : animStyle === '2d-flat' ? '- End every prompt with: 2D flat illustration, bold colors, clean vector art, smooth animation, no watermark, no text, no distortion.' : animStyle === '3d-realistic' ? '- End every prompt with: hyper-realistic 3D render, natural skin texture, subsurface scattering, real hair, cinematic lighting, shallow depth of field, indistinguishable from live action, no watermark, no text, no distortion.' : '- End every prompt with: 3D Pixar-style animation, soft lighting, vibrant colors, cinematic render, no watermark, no text, no distortion.'}`;
  } else {
